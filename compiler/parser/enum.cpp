...
{
	if (!tokens.Match(TokenType::KwEnum)) return NONMATCH();

	auto enumName = ParseName(tokens);
	if (!enumName) return ERROR("Expected an identifier");

	if (!tokens.Match(TokenType::LCurly)) return ERROR("Expected '{'");

	Name* entry = ParseName(tokens);
	if (!entry) return ERROR("Expected identifier");
	...
	if (argComma)
	{
		...
		if (!entry) return ERROR("Expected an identifier");
		...
		while (entry && argComma)
		{
			...
			if (argComma && !entry) return ERROR("Expected an identifier");
			if (entry)
				values.push_back(entry);
		}
	}
